...
This document outlines the chronological development of the Image Similarity Search architecture, breaking down the 27 key software engineering aspects into a structured timeline. It details the systematic approach and solutions implemented at each stage.
...
## Phase 6: Real-Time Pipelines & Production Solidification (Recent Additions)
*Focus: WebSockets, true two-way sync, UI persistence, and Docker orchestration for production.*

**21. WebSockets for Live UI Updates**
*   **Solution:** Upgraded the FastAPI backend with `uvicorn[standard]` to support a `/ws/gallery` endpoint. A background `asyncio` task listens to a Redis Pub/Sub channel and broadcasts MinIO indexing results and deletions directly to all connected users, providing instant live updates to the UI without requiring page refreshes or HTTP polling.

**22. Full Two-Way Sync (Multi-Select Delete UI)**
*   **Solution:** Created a two-way flow for object lifecycle management. Users can now select rows on the `/data` page and bulk-delete them via a new `DELETE /api/gallery` endpoint. This deletes objects from MinIO, which triggers the worker pipeline to clean the database and ping the WebSocket to clear the row from the UI for all users simultaneously.

**23. State Persistence and Caching in React**
*   **Solution:** Prevented loss of heavily computed search results and WebSocket connections when navigating between the Dashboard and Data pages by moving from dynamic `<Route>` component unmounting to an always-mounted design utilizing CSS `display: none/block`, heavily improving user experience across large searches.

**24. Correct Docker Production Separation**
*   **Solution:** Separated MinIO `ROOT_USER/PASSWORD` from the web app's `ACCESS_KEY/SECRET_KEY` to mimic real-world least privilege IAM configuration in [docker-compose.yml](file:///c:/Users/ACHAL.TRIVEDI/Downloads/InHouse-Indexed%20Search/docker-compose.yml). Configured the API workers to cleanly support long-lasting async WebSocket loops.

**25. ONNX Runtime CPU Optimization**
*   **Solution:** Migrated the embedding pipeline from pure PyTorch to ONNX Runtime. Created [tools/export_clip_onnx.py](file:///c:/Users/ACHAL.TRIVEDI/Downloads/InHouse-Indexed%20Search/tools/export_clip_onnx.py) to bake the NLP/Vision layers into a static computation graph ([clip-vit-base-patch32.onnx](file:///c:/Users/ACHAL.TRIVEDI/Downloads/InHouse-Indexed%20Search/models/clip-vit-base-patch32.onnx)).
*   **Impact:** Stripped the massive `torch` (~800MB) dependency out of the worker container, replacing it with the lightweight `onnxruntime` (~50MB). Inference speeds on CPU servers shot up significantly (handling ~100-200ms per image instead of 500ms+), heavily reducing the ingestion backlog during bulk MinIO bucket uploads.

**26. Highly-Responsive Data Page Search**
*   **Solution:** Added a debounced (400ms) text-search `<Input>` box to the Data page to filter the 300,000+ assets instantly.
*   **Mechanism:** Wired it directly to PostgreSQL's native `ILIKE` clause against the [object_key](file:///c:/Users/ACHAL.TRIVEDI/Downloads/InHouse-Indexed%20Search/core/ingestion_jobs.py#24-29) column on the `GET /api/gallery` endpoint, enabling lightning-fast partial string matching. Users can now isolate `.ai` files, `.pdf` files, or instantly inspect any randomly corrupted subdirectories without SQL access.
